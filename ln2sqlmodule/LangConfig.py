...
class LangConfig:
	...
	def load(self, path):
		with open(path) as f:
			...
			self.avg_keywords=[ self.remove_accents(a.strip()) for a in content[0].replace(':',',').split(",")]
			self.avg_keywords = self.avg_keywords[1:len(self.avg_keywords)]
			self.sum_keywords = [ self.remove_accents(a.strip()) for a in content[1].replace(':',',').split(",")]
			self.sum_keywords = self.sum_keywords[1:len(self.sum_keywords)]
			self.max_keywords = [ self.remove_accents(a.strip()) for a in content[2].replace(':',',').split(",")]
			self.max_keywords = self.max_keywords[1:len(self.max_keywords)]
			self.min_keywords = [ self.remove_accents(a.strip()) for a in content[3].replace(':',',').split(",")]
			self.min_keywords = self.min_keywords[1:len(self.min_keywords)]
			self.count_keywords = [ self.remove_accents(a.strip()) for a in content[4].replace(':',',').split(",")]
			self.count_keywords = self.count_keywords[1:len(self.count_keywords)]
			self.junction_keywords = [ self.remove_accents(a.strip()) for a in content[5].replace(':',',').split(",")]
			self.junction_keywords = self.junction_keywords[1:len(self.junction_keywords)]
			self.disjunction_keywords = [ self.remove_accents(a.strip()) for a in content[6].replace(':',',').split(",")]
			self.disjunction_keywords = self.disjunction_keywords[1:len(self.disjunction_keywords)]
			self.greater_keywords = [ self.remove_accents(a.strip()) for a in content[7].replace(':',',').split(",")]
			self.greater_keywords = self.greater_keywords[1:len(self.greater_keywords)]
			self.less_keywords = [ self.remove_accents(a.strip()) for a in content[8].replace(':',',').split(",")]
			self.less_keywords = self.less_keywords[1:len(self.less_keywords)]
			self.between_keywords = [ self.remove_accents(a.strip()) for a in content[9].replace(':',',').split(",")]
			self.between_keywords = self.between_keywords[1:len(self.between_keywords)]
			self.order_by_keywords = [ self.remove_accents(a.strip()) for a in content[10].replace(':',',').split(",")]
			self.order_by_keywords = self.order_by_keywords[1:len(self.order_by_keywords)]
			self.group_by_keywords = [ self.remove_accents(a.strip()) for a in content[11].replace(':',',').split(",")]
			self.group_by_keywords = self.group_by_keywords[1:len(self.group_by_keywords)]
			self.negation_keywords = [ self.remove_accents(a.strip()) for a in content[12].replace(':',',').split(",")]
